...
md`# Taxa de desemprego nos EUA em agosto de 2016 (Parte 2)`
  ...
  main.variable(observer("buildvis")).define("buildvis", ["d3","DOM","topojson","us","colorS","rateById","showTooltip","hideTooltip","county"], function(d3,DOM,topojson,us,colorS,rateById,showTooltip,hideTooltip,county)
{
  const width = 960
  const height = 600
...
d3.csv("https://gist.githubusercontent.com/emanueles/19cf3828bab232612c2b1599d831f690/raw/ddbaf62af22512a39360cc6741b40f5588b44aa0/county_names.csv").then(function (data){
  let countyP = d3.map()
  data.forEach(function(d) {
    countyP.set(d.id, d.name, d.state)
  })
  return countyP
})
)});
  main.variable(observer("tooltip")).define("tooltip", ["d3"], function(d3)
{
  ...
  let node = d3.select("body")
      ...
      .attr("class","hidden")
      .append("p")
      .html("<b><span id='city'></span></b><br>Taxa de desemprego: <span id='taxa'></span>%")
  return node
}
);
